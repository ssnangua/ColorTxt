...
└── renderer/
    ...
    └── src/
        ├── main.ts           # Vue 应用挂载
        ├── App.vue           # 根组件：布局与全局状态串联；书钉/书签、全屏阅读区布局、阅读进度等拆至 composables；向 ReaderMain 传入阅读偏好；维护 `shortcutBindings` 并传给 `AppHeader`（更多菜单快捷键与 `shortcutService` 一致）；挂载 `AppOverlays` 等
...
| 文件                                                 | 主要功能                                                                                                                                                                                                                                                                                                                                                                                   |
| ---------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `AppHeader.vue`                                      | 顶栏：打开文件、书钉/书签、字体与字号行高、压缩空行/行首缩进、**高级换行策略**（Monaco `wrappingStrategy: advanced`）、内容上色、章节规则、主题、侧栏与全屏、查找与更多菜单等；从 `App.vue` 接收当前 **`shortcutBindings`** 并传给 `MoreMenu`，使「更多」里展示的快捷键与设置一致                                                                                                                                                                                                                                                                                                                                                                 |
...
| `ColorSchemePanel.vue`                               | 配色弹窗：`ReaderSurfacePalette` 亮/暗两套草稿、阅读器色表与实时预览（`HexColorPickerField`）；确定时 `applyReaderPalettes` 写回 `App.vue` 并持久化；含「关键词」页签与「阅读器」页签                                                                                                                                                                                                                                                                                                                                                                     |
| `HexColorPickerField.vue`                            | 单行十六进制颜色 + HSV 取色浮层（智能上下翻转、视口贴边）；`draftHex` / `draftEnd` 事件供父组件在弹层打开期间做临时预览                                                                                                                                                                                                                                                                                                                                                                     |
| `MoreMenu.vue`                                       | 更多菜单：最近文件、查找、快捷键、设置、配色、检查更新、关于、退出等；菜单项右侧快捷键文案来自 **`shortcutBindings`**，经 `shortcutUtils.acceleratorToDisplayText` 与快捷键面板及 `shortcutService` 实际生效绑定同步                                                                                                                                                                                                                                                                                                                             |
...
| 键名                     | 大致内容                                                                                                                                                                                                                                                                                                                                       |
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `colorTxt.ui.settings`  | 界面与阅读偏好：字体、字号与行高倍数，空行压缩/行首缩进、高级换行、内容着色，章节匹配规则、主题、侧栏是否展开，侧栏的宽度、章节字数显示，设置里的启动是否恢复会话、最近文件条数上限、全屏正文区宽度，**可选字段 `shortcutBindings`**（各快捷键动作 ID → 快捷键字符串，缺省则使用 
